...
def generator(noise_dim=NOISE_DIM, seed=None):
    ...
    model = nn.Sequential(
      ...
      nn.Linear(1024, 784),
      nn.Tanh(),
      # run_a_gan: logits_fake = D(fake_images.view(batch_size, 1, 28, 28))
      Unflatten(N=128, C=1, H=28, W=28),
    )
...
def discriminator_loss(logits_real, logits_fake):
    ...
    labels_fake = torch.zeros_like(logits_real).type(dtype)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    real_loss = 0.5 * torch.mean(
      (scores_real - 1) ** 2
    )
    fake_loss= 0.5 * torch.mean(
      scores_fake ** 2
    )
